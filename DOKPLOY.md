...
3) Definis le contexte de build sur `apps/web` (important pour les `COPY` du Dockerfile).
4) Configure les variables d'environnement (voir section ci-dessous).
5) Monte un volume persistant pour `/app/storage` afin de conserver les PDF et logos.
6) Declenche un deploy.
...
- `ADMIN_EMAIL` (creation du compte admin initial)
- `ADMIN_PASSWORD` (creation du compte admin initial)
- `OCR_LLM_ENABLED`
- `OCR_LLM_PROVIDER`
